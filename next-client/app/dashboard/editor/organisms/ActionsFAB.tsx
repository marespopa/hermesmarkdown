...
const ActionsFAB: React.FC<ActionsFABProps> = ({ actions, exportToMD }) => {
  ...
  return (
    <div
      className="fixed top-2 right-4 sm:right-6 z-[100] flex flex-col items-end gap-2 font-mono selection:bg-zinc-200 dark:selection:bg-zinc-800"
      ref={menuRef}
    >
      {/* Control Strip */}
      ...
      >
        <div className="w-48 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-xl p-1.5 flex flex-col gap-0.5">
          ...
          <MenuButton
            onClick={() => {
              actions.handleOpenFindAndReplace();
              setIsOpen(false);
            }}
