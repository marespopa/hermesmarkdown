...
export function useMarkdownEditor({
  ...
  // The global voice-input hook (see use-global-voice-input.ts) bumps this
  // when a "insert link" voice command fires. Only the active pane's own
  // link dialog should pop open in response.
  const voiceOpenLinkDialogRequest = useAtomValue(atom_voiceOpenLinkDialogRequest);
  const prevVoiceOpenLinkDialogRequestRef = useRef(voiceOpenLinkDialogRequest);
  useEffect(() => {
    if (voiceOpenLinkDialogRequest !== prevVoiceOpenLinkDialogRequestRef.current) {
      prevVoiceOpenLinkDialogRequestRef.current = voiceOpenLinkDialogRequest;
      if (isActivePane) setLinkDialogOpen(true);
    }
  }, [voiceOpenLinkDialogRequest, isActivePane, setLinkDialogOpen]);
  ...
  // The global voice-input hook (see use-global-voice-input.ts) is a single
  // instance shared by the whole app, not one per pane. It inserts a
  // committed dictation into whichever textarea this atom currently points
  // at, so this pane only needs to claim that slot while it's the active one.
  const setActiveTextareaElement = useSetAtom(atom_activeTextareaElement);
  useEffect(() => {
    if (isActivePane && textareaMounted) {
      setActiveTextareaElement(textareaRef.current);
    }
